...
{!! $dataTable->scripts() !!}
<!--DataTables js-->
@include('Hospital.pages.invoices.invoices.components.extends.ajax.container')
<script>
    $('#reload_table').on('click',function () {
        $('#invoice_table').DataTable().ajax.reload(null, false);
    })
    let status = "{{ \Illuminate\Support\Facades\Session::get('status') }}";
    if(status === 'success')
    {
        @include('Hospital.pages.messages.successMessage',['msg'=>'تم الدفع بنجاح'])
    }
