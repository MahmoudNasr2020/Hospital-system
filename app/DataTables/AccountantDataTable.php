...
class AccountantDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action',function($data){
                return '<a href="'.route('hospital.accountant.show',['id'=>$data->id,'name'=>str_replace(' ','_',$data->user->name)]).'">
                             <i title="Show" class="fa fa-eye showing"
                            data-id="'.$data->id.'" style="cursor: pointer;color:blue;font-size:17px"></i>
                        </a>
                               <a href="'.route('hospital.accountant.edit',['id'=>$data->id,'name'=>str_replace(' ','_',$data->user->name)]).'">
                                <i title="Edit"
                                class="fa fa-pencil edit" data-id="'.$data->id.'" data-route="'.route('hospital.accountant.edit',$data->id).'"
                                style="cursor: pointer;color:green;font-size:17px"></i></a>
                                <i data-bs-toggle="tooltip" data-bs-placement="bottom" title="Delete" class="fa fa-trash delete" data-id="'.$data->id.'" data-route="'.route('hospital.accountant.delete',$data->id).'" style="cursor: pointer;color:#ff0000;font-size:17px"></i>';
    ...
    public function query(Accountant $model)
    {
        return Accountant::query()->with(['user'])->select('accountants.*');
    }

    public function html()
    {
        return $this->builder()
            ->setTableId('accountant_table')
    ...
    {
        return 'Accountant_' . date('YmdHis');
    }
}
