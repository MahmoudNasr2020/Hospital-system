...
class NurseDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action',function($data){
                return '<a href="'.route('hospital.nurse.show',['id'=>$data->id,'name'=>str_replace(' ','_',$data->user->name)]).'">
                             <i title="Show" class="fa fa-eye showing"
                            data-id="'.$data->id.'" style="cursor: pointer;color:blue;font-size:17px"></i>
                        </a>

                               <a href="'.route('hospital.nurse.edit',['id'=>$data->id,'name'=>str_replace(' ','_',$data->user->name)]).'">
                                <i title="Edit"
                                class="fa fa-pencil edit" data-id="'.$data->id.'" data-route="'.route('hospital.nurse.edit',$data->id).'"
                                style="cursor: pointer;color:green;font-size:17px"></i></a>
                                <i data-bs-toggle="tooltip" data-bs-placement="bottom" title="Delete" class="fa fa-trash delete" data-id="'.$data->id.'" data-route="'.route('hospital.nurse.delete',$data->id).'" style="cursor: pointer;color:#ff0000;font-size:17px"></i>';
    ...
    {
        return Nurse::query()->with(['department','user'])->select('nurses.*');
    }

    public function html()
    {
        return $this->builder()
            ->setTableId('nurse_table')
