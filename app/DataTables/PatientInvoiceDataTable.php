...
class PatientInvoiceDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action',function($data){
                return '
                  <a href="'.route('hospital.invoice.create',['id'=>$data->id,'name'=>str_replace(' ','_',$data->name)]).'" target="_blank">
                                <i title="اضافة فاتورة"
                                class="fa fa-plus add" data-id="'.$data->id.'"
                                style="cursor: pointer;color:brown;font-size:17px"></i>
                                </a>

                                <a href="'.route('hospital.invoice.index',['id'=>$data->id,'name'=>str_replace(' ','_',$data->name)]).'" target="_blank">
                                <i title="عرض الفواتير"
                                class="fa fa-file-invoice-dollar show_report" data-id="'.$data->id.'"
    ...
    {
        return $this->builder()
            ->setTableId('patientInvoice_table')
    ...
    {
        return 'PatientInvoice_' . date('YmdHis');
    }
}
