...
class DoctorDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action',function($data){
                return '<a href="'.route('hospital.doctor.show',['id'=>$data->id,'name'=>str_replace(' ','_',$data->user->name)]).'">
                             <i title="Show" class="fa fa-eye showing"
                            data-id="'.$data->id.'" style="cursor: pointer;color:blue;font-size:17px"></i>
                        </a>

                               <a href="'.route('hospital.doctor.edit',['id'=>$data->id,'name'=>str_replace(' ','_',$data->user->name)]).'">
                                <i title="Edit"
                                ...
                                <i data-bs-toggle="tooltip" data-bs-placement="bottom" title="Delete" class="fa fa-trash delete" data-id="'.$data->id.'"
                                 data-route="'.route('hospital.doctor.delete',$data->id).'" style="cursor: pointer;color:#ff0000;font-size:17px"></i>';
    ...
    {
        return Doctor::query()->with(['department','user'])->select('doctors.*');
    }

    public function html()
    {
        return $this->builder()
            ->setTableId('doctor_table')
